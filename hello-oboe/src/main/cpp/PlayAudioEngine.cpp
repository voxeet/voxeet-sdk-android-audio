...
void PlayAudioEngine::createPlaybackStream() {
    ...
    if (result == oboe::Result::OK && mPlayStream != nullptr) {
        ...
        prepareOscillators(mFramesPerBurst);
...
void PlayAudioEngine::prepareOscillators(size_t buffer_length) {
...
bool PlayAudioEngine::getToneOn() {
    return mIsToneOn;
}
...
PlayAudioEngine::onAudioReady(oboe::AudioStream *audioStream, void *audioData, int32_t numFrames) {
    ...
    /*float time = 0.0;
    data_t azim_deg = 0.0;
    data_t distance_m = 0.0;*/
    //float frequency = 1.0/60.0; // period of 10sec.

    // If the tone is on we need to use our synthesizer to render the audio data for the sine waves
    if (audioStream->getFormat() == oboe::AudioFormat::Float) {
        if (mIsToneOn) {
            for (int i = 0; i < 1; ++i) {
                //int64_t  framesWritten = audioStream->getFramesWritten();
